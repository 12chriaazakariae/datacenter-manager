...
class DatabaseSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        // Ajout de matériel supplémentaire pour remplir les stats
        for ($i = 1; $i <= 12; $i++) {
            \App\Models\Resource::create([
                'name' => "Serveur Rack Enterprise v$i",
                'category_id' => $catServeurs->id,
                'description' => "Serveur $i pour le clustering.",
                'specs' => 'RAM: 128GB, CPU: 32 Cores',
                'state' => 'active'
            ]);
        }
        ...
        for ($i = 1; $i <= 25; $i++) {
            \App\Models\Resource::create([
                'name' => "Poste Travail Dev $i",
                'category_id' => $catLaptops->id,
                'description' => 'Laptop de remplacement.',
                'specs' => 'RAM: 32GB, CPU: i9',
                'state' => 'active'
            ]);
        }
        ...
        for ($i = 1; $i <= 15; $i++) {
            \App\Models\Resource::create([
                'name' => "Switch Edge $i",
                'category_id' => $catSwitchs->id,
                'description' => 'Switch de bordure réseau.',
                'specs' => 'SFP+, 48 ports',
                'state' => 'active'
            ]);
        }
